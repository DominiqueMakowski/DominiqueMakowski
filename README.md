# Hello there, I'm Dom 👋

[![GitHub followers](https://img.shields.io/github/followers/DominiqueMakowski?label=Follow%20me&style=flat-square&logo=github&logoColor=white&colorB=4CAF50)](https://github.com/login?return_to=%2FDominiqueMakowski)
[![CV](https://img.shields.io/badge/CV-D._Makowski-purple.svg?colorB=9C27B0&style=flat-square)](https://dominiquemakowski.github.io/cv/)
[![Twitter Follow](https://img.shields.io/twitter/follow/Dom_Makowski?label=%20%40Dom_Makowski&style=flat-square&labelColor=2196F3&logo=twitter&logoColor=white&colorB=0D47A1)](https://twitter.com/Dom_Makowski)
[![Research gate](https://img.shields.io/badge/-Research%20Gate-green.svg?style=flat-square&logo=researchgate&logoColor=white&colorB=616161&labelColor=00BFA5)](https://www.researchgate.net/profile/Dominique_Makowski)


I'm mostly involved in projects related to data analysis and neuroscience. Python and R are my go-to languages, but I also dabble in Julia and Javascript.


- My CV, publications and all are available on my website: <a href="https://dominiquemakowski.github.io/"><b>dominiquemakowski.github.io</b></a> 
- My Lab website: <a href="https://realitybending.github.io/"><b>realitybending.github.io</b></a>


Most of these projects were done without funding on my own freetime. If they helped you, consider [**sponsoring**](https://github.com/sponsors/DominiqueMakowski) me with for instance a one time donation.


## Main projects

- [x] I am the cofounder of the [**easystats**](https://github.com/easystats/) collection of R packages
- [x] Creator of [**NeuroKit2**](https://github.com/neuropsychology/NeuroKit), the most popular Python toolbox for neurophysiological signal processing
- [x] My [**teaching materials**](https://github.com/DominiqueMakowski/teaching) are open source


## Data Analysis

- [x] [**cogmod**](https://github.com/DominiqueMakowski/cogmod): An R package that implements cognitive computational models
- [x] [**SequentialSamplingModels.jl**](https://github.com/itsdfish/SequentialSamplingModels.jl): Contributor to a Julia package for RT models

## Neuropsychology

- [x] [**Pyllusion**](https://github.com/RealityBending/Pyllusion): 🤯 A Python module for generating visual illusions
- [x] [**PyShaman**](https://github.com/RealityBending/PyShaman): 🧙‍♂️ A Python software for inducing altered states of consciousness
- [x] [**Neuropsydia**](https://github.com/neuropsychology/Neuropsydia.py): A Python Module for Creating Experiments, Tasks and Questionnaires.
- [x] [**CognitiveControl**](https://github.com/neuropsychology/CognitiveControl): A neuropsychological task measuring different aspects of cognitive control (processing speed, inhibition, conflict resolution...)
- [x] [**RestingState**](https://github.com/neuropsychology/RestingState): Standard resting state protocol with brief mental state assessment
- [x] [**PatientAssessmentApp**](https://github.com/neuropsychology/PatientAssessmentApp): Easy Neuropsychological Assessment of Patients Scores and Deficits

## Art

- [x] [**NeuropsyXart**](https://dominiquemakowski.github.io/NeuropsyXart): Body & Mind Art
- [x] [**OneAttractor**](https://github.com/DominiqueMakowski/OneAttractor): One chaotic attractor per day
- [x] [**Music**](https://github.com/DominiqueMakowski/Music): My arrangements and compositions (for Piano)
- [x] [**Dom's tabs**](https://dominiquemakowski.github.io/Music/tabs/): Web app for displaying chords and lyrics


## Utilities

- [x] [**OpenMuse**](https://github.com/DominiqueMakowski/OpenMuse): The first Open Python Muse S Athena EEG Decoder
- [x] [**MuseLSL2**](https://github.com/DominiqueMakowski/MuseLSL2): muse-lsl interface to stream and record Muse EEG signals
- [x] [**TruScanEEGpy**](https://github.com/neuropsychology/TruScanEEGpy): Utility functions to work with Deymed's TruScan EEG system


## Random, Unifinished or Old

- [x] [**Occultopedia**](https://github.com/DominiqueMakowski/Occultopedia): a database and webapp to explore occult movements and secret societies
- [x] [**StatsAndSorcery**](https://github.com/DominiqueMakowski/StatsAndSorcery): a educational game to develop the intuition behind linear models
- [x] [**Lab calendar**](https://github.com/RealityBending/Calendar): a fun semi-fictional calendar that automatically updates a ggplot-made clock daily
- [x] [**psycho**](https://github.com/neuropsychology/psycho.R): My first R package, meant for experimental psychologists
- [x] [**Psycho.jl**](https://github.com/neuropsychology/Psycho.jl): An attempt of implementing *psycho* in Julia
- [x] [**popularipy**](https://github.com/DominiqueMakowski/popularipy): A Python package to access popularity metrics for your package
- [x] [**Mythpsychology**](https://github.com/DominiqueMakowski/Mythpsychology): A review of existing relationship between myths and psychological concepts
- [x] [**tidyfamily**](https://github.com/DominiqueMakowski/tidyfamily): An R package to easily build and display genealogical trees using R

---



*2022/02/01* Check-out my discussion on the **Learning Bayesian Statistics** podcast:

<a href="https://www.learnbayesstats.com/episode/55-neuropsychology-illusions-bending-reality-dominique-makowski"><img src='https://realitybending.github.io/post/2022-02-01-learnbayesstats/featured_hu14067991586290034931.webp' align="left" height="200" /></a>
<br><br><br><br><br><br><br><br><br>








